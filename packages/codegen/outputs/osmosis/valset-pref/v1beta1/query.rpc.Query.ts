...
export const createRpcQueryMobxStores = (rpc: ProtobufRpcClient | undefined) => {
  const queryService = getQueryService(rpc);
  class QueryUserValidatorPreferencesStore {
    store = new QueryStore<UserValidatorPreferencesRequest, UserValidatorPreferencesResponse>(queryService?.userValidatorPreferences);
    userValidatorPreferences(request: UserValidatorPreferencesRequest) {
      return this.store.getData(request);
    }
  }
  return {
    /** Returns the list of ValidatorPreferences for the user. */QueryUserValidatorPreferencesStore
  };
};