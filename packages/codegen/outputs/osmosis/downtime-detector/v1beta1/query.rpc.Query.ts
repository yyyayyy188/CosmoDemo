...
export const createRpcQueryMobxStores = (rpc: ProtobufRpcClient | undefined) => {
  const queryService = getQueryService(rpc);
  class QueryRecoveredSinceDowntimeOfLengthStore {
    store = new QueryStore<RecoveredSinceDowntimeOfLengthRequest, RecoveredSinceDowntimeOfLengthResponse>(queryService?.recoveredSinceDowntimeOfLength);
    recoveredSinceDowntimeOfLength(request: RecoveredSinceDowntimeOfLengthRequest) {
      return this.store.getData(request);
    }
  }
  return {
    QueryRecoveredSinceDowntimeOfLengthStore
  };
};